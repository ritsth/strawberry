...
class GraphQLCoreConverter:
    ...
    def from_resolver(
            ...
            def extension_resolver(
                ...
                **kwargs: Any,
                ...
                try:
                    ...
                    # Argument conversion failed before the resolver or any field
                    # extension could run. If a handler matches, map the exception
                    # to a union result directly; the field-extension chain is
                    # skipped, matching how conversion errors have always bypassed
                    # field extensions such as permissions. ``_handle_exception``
                    # returns ``UNHANDLED`` when no handler applies (including when
                    # the field has none), so the error otherwise propagates as
                    # before.
                    handled = _handle_exception(exc, info)
                    if handled is UNHANDLED:
                        raise
                ...
                def wrapped_get_result(_source: Any, info: Info, **kwargs: Any) -> Any:
                    ...
                    # Convert the resolver's own error here, at the innermost
                    # point of the field-extension chain, before any extension
                    # can observe or transform it. Errors raised by the field
                    # extensions themselves get their chance at the outermost
                    # layer, in `_resolver`/`_async_resolver`.
        ...
        # Exceptions raised by the field extensions themselves (permission
        # checks, validation extensions, ...) are converted at the outermost
        # layer below. Without extensions the innermost handling already sees
        # every exception, so the extra wrapping is skipped.
        handle_extension_errors = bool(field_exception_handlers and field.extensions)

        def _resolver(_source: Any, info: GraphQLResolveInfo, **kwargs: Any) -> Any:
            strawberry_info = _strawberry_info_from_graphql(info)

            if not handle_extension_errors:
